    ...
    <div className="mx-auto min-w-0 max-w-3xl px-0 pb-10 sm:px-1">
      ...
      {user.gradesheetImport?.records?.length > 0 && (
        <details className="mb-3 rounded-lg border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-400">
          <summary className="cursor-pointer font-medium text-neutral-200">{user.gradesheetImport.source === "connect" ? "Imported Connect history" : "Imported grade history"}</summary>
          <p className="mt-2">{user.gradesheetImport.note || "Original grades are retained here. Planner edits and undoing completion do not change the uploaded academic record."}</p>
